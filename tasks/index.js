...
const { MINICHEF_ADDRESS } = require("@123swap/core-sdk-v2")
...
task("OneTwoThreeMasterChef:add", "Add pool to OneTwoThreeMasterChef")
.setAction(async function (taskArguments, { ethers: { getNamedSigner } }, runSuper) {
  ...
  await (await OneTwoThreeMasterChef.connect(await getNamedSigner("dev")).add(1000, '0x3e78a806b127c02b54419191571d9379819e989c', true)).wait()
});

task("OneTwoThreeMasterChef:deposit", "OneTwoThreeMasterChef deposit")
.addParam("pid", "Pool ID")
.addParam("amount", "Amount")
.setAction(async function ({ pid, amount }, { ethers: { getNamedSigner } }, runSuper) {
  const OneTwoThreeMasterChef = await ethers.getContract("OneTwoThreeMasterChef")

  const { lpToken } = await OneTwoThreeMasterChef.poolInfo(pid)

  await run("erc20:approve", { token: lpToken, spender: OneTwoThreeMasterChef.address })

  await (await OneTwoThreeMasterChef.connect(await getNamedSigner("dev")).deposit(pid, amount)).wait()
});

task("OneTwoThreeMasterChef:withdraw", "OneTwoThreeMasterChef withdraw")
.addParam("pid", "Pool ID")
.addParam("amount", "Amount")
.setAction(async function ({ pid, amount }, { ethers: { getNamedSigner } }, runSuper) {
  const OneTwoThreeMasterChef = await ethers.getContract("OneTwoThreeMasterChef")
  ...
  await (await OneTwoThreeMasterChef.connect(await getNamedSigner("dev")).withdraw(pid, amount)).wait()
});
...
.setAction(async function ({ rewardToken }, { ethers: { getNamedSigner }, getChainId, deployments }, runSuper) {
  ...
  if (chainId === "31337") {
    miniChefAddress = (await deployments.get("OneTwoThreeMiniChefV2")).address;
