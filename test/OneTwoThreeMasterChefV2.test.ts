...
describe("OneTwoThreeMasterChefV2", function () {
  before(async function () {
    await prepare(this, ["OneTwoThreeMasterChef", "OneTwoThreeBonusToken", "ERC20Mock", "OneTwoThreeMasterChefV2", "RewarderMock"])
  })

  beforeEach(async function () {
    await deploy(this, [["bonus", this.OneTwoThreeBonusToken]])

    await deploy(this, [
      ["lp", this.ERC20Mock, ["LP Token", "LPT", getBigNumber(10)]],
      ["dummy", this.ERC20Mock, ["Dummy", "DummyT", getBigNumber(10)]],
      ["chef", this.OneTwoThreeMasterChef, [this.bonus.address, this.bob.address, getBigNumber(100), "0", "0"]],
    ])
    ...
    await deploy(this, [
      ["chef2", this.OneTwoThreeMasterChefV2, [this.chef.address, this.bonus.address, 1]],
  ...
  describe("PendingBonus", function () {
    it("PendingBonus should equal ExpectedBonus", async function () {
      await this.chef2.add(10, this.rlp.address, this.rewarder.address)
      ...
      await advanceBlock()
      let expectedBonus = getBigNumber(100)
        .mul(log2.blockNumber + 1 - log.blockNumber)
        .div(2)
      let pendingBonus = await this.chef2.pendingBonus(0, this.alice.address)
      expect(pendingBonus).to.be.equal(expectedBonus)
    })
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef2.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef2.poolInfo(0)).accBonusPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of BONUS and reward", async function () {
      ...
      await this.chef2.harvestFromOneTwoThreeMasterChef()
      let log2 = await this.chef2.withdraw(0, getBigNumber(1), this.alice.address)
      let expectedBonus = getBigNumber(100)
        .mul(log2.blockNumber - log.blockNumber)
        .div(2)
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedBonus)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.bonus.balanceOf(this.alice.address))
        .to.be.equal(await this.r.balanceOf(this.alice.address))
        .to.be.equal(expectedBonus)
    })
    ...
    it("Harvest for BONUS-only pool", async function () {
      ...
      expect((await this.chef2.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedBonus)
      await this.chef2.harvest(0, this.alice.address)
      expect(await this.bonus.balanceOf(this.alice.address)).to.be.equal(expectedBonus)
    })
  })
