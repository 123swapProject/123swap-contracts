...
describe("OneTwoThreeMiniChefV2", function () {
  before(async function () {
    await prepare(this, ["OneTwoThreeMiniChefV2", "OneTwoThreeBonusToken", "ERC20Mock", "RewarderMock"])
  })
  ...
  beforeEach(async function () {
    ...
    await deploy(this, [
      ...
      ["chef", this.OneTwoThreeMiniChefV2, [this.bonus.address]],
    ...
    await this.bonus.mint(this.chef.address, getBigNumber(10000))
    await this.lp.approve(this.chef.address, getBigNumber(10))
    await this.chef.setBonusPerSecond("10000000000000000")
  ...
  describe("PendingBonus", function () {
    it("PendingBonus should equal ExpectedBonus", async function () {
      ...
      let expectedBonus = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      let pendingBonus = await this.chef.pendingBonus(0, this.alice.address)
    ...
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chef.updatePool(0))
        ...
        .withArgs(
          ...
          (await this.chef.poolInfo(0)).accBonusPerShare
        )
    })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of BONUS and reward", async function () {
      ...
      let timestamp = (await ethers.provider.getBlock(log.blockNumber)).timestamp
      let expectedBonus = BigNumber.from("10000000000000000").mul(timestamp2 - timestamp)
      expect((await this.chef.userInfo(0, this.alice.address)).rewardDebt).to.be.equal("-" + expectedBonus)
